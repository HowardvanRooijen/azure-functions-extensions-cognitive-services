namespace AzureFunctions.Extensions.CognitiveServices.Bindings.Vision.Describe
{
    ...
    public class VisionDescribeBinding : IExtensionConfigProvider, IVisionBinding
    {
        ...
        public void Initialize(ExtensionConfigContext context)
        {
            ...
            visionDescribeRule.When(nameof(VisionDescribeAttribute.ImageSource), ImageSource.BlobStorage).BindToInput<VisionDescribeModel>(GetVisionDescribeModel);
            visionDescribeRule.When(nameof(VisionDescribeAttribute.ImageSource), ImageSource.Url).BindToInput<VisionDescribeModel>(GetVisionDescribeModel);
            visionDescribeRule.When(nameof(VisionDescribeAttribute.ImageSource), ImageSource.Client).BindToInput<VisionDescribeClient>(attr => new VisionDescribeClient(this, attr, this.loggerFactory));
        }
        ...
        private void LoadClient()
        {
            if (Client == null)
            {
                Client = new CognitiveServicesClient(new RetryPolicy(), this.loggerFactory);
            }
        }
        ...
        private VisionDescribeModel GetVisionDescribeModel(VisionDescribeAttribute attribute)
        {
            ...
            var client = new VisionDescribeClient(this, attribute, this.loggerFactory);
            ...
            var task = client.DescribeAsync(request);
